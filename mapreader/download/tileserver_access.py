...
class TileServer:
    def __init__(self, metadata_path, geometry="polygone", 
    ...
    def create_info(self):
        ...
        for one_item in self.metadata:
            min_lon, max_lon, min_lat, max_lat = \
                self.detect_rectangle_boundary(one_item["geometry"]["coordinates"][0][0])
            # XXX hardcoded: the file path is hardcoded to map_<IMAGE>.png
            metadata_info_list.append(
                [one_item["properties"]["IMAGEURL"], "map_" + one_item["properties"]["IMAGE"] + ".png"]
            )
            # Collect boundaries for fast queries
            metadata_coord_arr.append(
                [min_lon, max_lon, min_lat, max_lat]
            )
    ...
    def query_point(self, latlon_list, append=False):
        ...
        for one_q in latlon_list:
            lat_q, lon_q = one_q
            indx_q = np.where((self.metadata_coord_arr[:, 2] <= lat_q) & 
                              (lat_q <= self.metadata_coord_arr[:, 3]) & 
                              (self.metadata_coord_arr[:, 0] <= lon_q) & 
                              (lon_q <= self.metadata_coord_arr[:, 1]))[0]
            if len(indx_q) == 0:
            ...
            if not already_in_candidates:
                ...
                found_queries[-1].extend([copy.deepcopy(self.metadata_coord_arr[indx_q]), indx_q])
    ...
    def minmax_latlon(self):
        ...
        if not self.detected_rect_boundary:
            self.create_info()
        print(f"Min/Max Lon: {np.min(self.metadata_coord_arr[:, 0])}, {np.max(self.metadata_coord_arr[:, 1])}")
        print(f"Min/Max Lat: {np.min(self.metadata_coord_arr[:, 2])}, {np.max(self.metadata_coord_arr[:, 3])}")
    
    def download_tileserver(self, 
                            mode="queries", 
                            num_img2test=-1, 
                            zoom_level=14, 
                            adjust_mult=0.005, 
        ...
        for one_item in metadata:
            try:
                ...
                print("-------------------")
                output_stitcher = os.path.join(output_maps_dirname, "map_" + one_item["properties"]["IMAGE"] + ".png")

                if saved_metadata is not None:

                    try_cond1 = str(one_item["geometry"]["coordinates"][0][0]) in saved_metadata["polygone"].to_list()
                    try_cond2 = f'map_{one_item["properties"]["IMAGE"]}.png' in saved_metadata["name"].to_list()

                    if (not try_cond1) and (try_cond2):
                        append_id = int(one_item["id"].split(".")[1])
                        output_stitcher = os.path.join(output_maps_dirname, f'map_{one_item["properties"]["IMAGE"]}@{append_id}.png')
                ...
                if (not try_cond1) and (try_cond2):
                    ...
                    poly_filename = f'{one_item["properties"]["IMAGE"]}_{counter}.geojson'
                ...
                # Run the scraper
                scraper(os.path.join("geojson", poly_filename), 
                        zoom_level, 
                        self.download_url, 
                        tile_tmp_dir, 
                ...
                name_region, _, _, published_date = self.extract_region_dates_metadata(one_item) 
    ...
    def hist_published_dates(self, min_date=None, max_date=None):
        ...
        print(f"Min/Max published dates: {min(all_published_date)}, {max(all_published_date)}")
        if min_date == None:
            min_date = min(all_published_date)
        if max_date == None:
            max_date = max(all_published_date)
        plt.hist(all_published_date, 
                 align='mid', 
    ...
    def download_tileserver_rect(self, 
                                 mode="queries", 
                                 num_img2test=-1, 
                                 zoom_level=14, 
                                 adjust_mult=0.005, 
        ...
        for one_item in metadata:
            ...
            while not_saved:
                ...
                poly_filename = one_item["properties"]["IMAGE"] + "_{}.geojson".format(counter)
                fio = open(os.path.join("geojson", poly_filename), "w")
                fio.writelines(str2write)
                fio.close()
                ...
                # Run the scraper
                scraper(os.path.join("geojson", poly_filename), zoom_level, self.download_url, tile_tmp_dir, retries=retries)
                ...
                if os.path.isfile(failed_urls_path):
                    with open(failed_urls_path, "r") as fp:
                        errors = fp.readlines()
                        if len(errors) > 0:
                            print(f"[WARNING] Could not retrieve {len(errors)} tiles, changing the coordinates")
            ...
            pd_info.loc[counter] = [os.path.basename(output_stitcher), 
...
def download_tileserver_parallel(metadata, start, end, process_np=8, **kwds):
    ...
    for index in range(req_proc):
        ...
        p = multiprocessing.Process(target=download_tileserver,
                                    args=(metadata_one_node,
                                          kwds["geometry"], 
                                          kwds["num_img2test"], 
                                          kwds["download_url"], 
                                          kwds["zoom_level"], 
