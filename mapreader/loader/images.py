...
class mapImages:
    ...
    def __init__(self, 
        ...
        for one_image_path in self.path_images:
            self.imagesConstructor(image_path=one_image_path, 
                                   parent_path=parent_path,
                                   tree_level=tree_level,
                                   **kwds) 

    def imagesConstructor(self, image_path, parent_path=None, tree_level="child", **kwds):
        ...
        if tree_level not in ["parent", "child"]:
            raise ValueError(f"[ERROR] tree_level should be set to parent or child, current value: {tree_level}")
        if (parent_path is not None) and (tree_level == "parent"):
            raise ValueError(f"[ERROR] if tree_level=parent, parent_path should be None.")
    ...
    def add_metadata(self, metadata, columns=None, tree_level="parent", index_col=0, delimiter="|"):
        ...
        if isinstance(metadata, pd.DataFrame):
            ...
            if index_col >= 0:
                metadata_df = pd.read_csv(metadata, index_col=index_col, delimiter=delimiter)
            else:
                metadata_df = pd.read_csv(metadata, index_col=False, delimiter=delimiter)
        else:
            raise ValueError(f"metadata should either path to a csv file or pandas dataframe.")
    ...
    def add_coord_increments_id(self, image_id, tree_level="parent"):
        ...
        if not "coord" in self.images[tree_level][image_id].keys():
            raise ValueError(f"'coord' could not be found in: self.images[tree_level][image_id].keys(). Suggestion: run add_metadata")
    ...
    def add_center_coord_id(self, image_id, tree_level="child"):
        ...
        if par_id is not None:
            ...
            self.images[tree_level][image_id]["center_lon"] = lon_min + (min_abs_x + max_abs_x) / 2.
            self.images[tree_level][image_id]["center_lat"] = lat_max - (min_abs_y + max_abs_y) / 2. 
        else:
            print(f"[INFO] parent_id is None. Read 'coord' from the specified tree_level and image_id.")
            lon_min, lon_max, lat_min, lat_max = self.images[tree_level][image_id]["coord"]

            self.images[tree_level][image_id]["center_lon"] = (lon_min + lon_max) / 2.
            self.images[tree_level][image_id]["center_lat"] = (lat_min + lat_max) / 2.
    ...
    def calc_pixel_width_height(self, parent_id, calc_size_in_m="great-circle"):
        ...
        if calc_size_in_m == 'geodesic':
            ...
            mean_width = np.mean([size_in_m[0]/myimg_shape[1], size_in_m[1]/myimg_shape[1]])
            mean_height = np.mean([size_in_m[2]/myimg_shape[0], size_in_m[3]/myimg_shape[0]])
    ...
    def sliceAll(self, 
        ...
        for one_image in img_keys:
            sliced_images_info = self._slice(image_path=self.images[tree_level][one_image]["image_path"], 
                                             method=method,
            ...
            if add2child:
                for i in range(len(sliced_images_info)):
                    ...
                    self.imagesConstructor(image_path=sliced_images_info[i][0], 
    ...
    def _slice(self, 
        ...
        if method == "pixel":
            sliced_images_info = \
                sliceByPixel(image_path=image_path, 
                             slice_size=slice_size,
                             path_save=path_save,
                             square_cuts=square_cuts,
                             resize_factor=resize_factor,
            ...
            if "tiff" in image_path:
                xmin, xmax, ymin, ymax, tiff_shape, size_in_m = \
                        utils.extractGeoInfo(image_path=image_path,
                ...
                xmin, xmax, ymin, ymax, img_shape, size_in_m = \
    ...
    def calc_pixel_stats(self, parent_id=None, calc_mean=True, calc_std=True):
        ...
        for one_par_id in parent_id:
            ...
            for one_child in list_children:
                ...
                if calc_std:
                    self.images["child"][one_child]["std_pixel_R"]   = np.std(child_img[:, :, 0])
                    self.images["child"][one_child]["std_pixel_G"]   = np.std(child_img[:, :, 1])
                    self.images["child"][one_child]["std_pixel_B"]   = np.std(child_img[:, :, 2])
                    self.images["child"][one_child]["std_pixel_RGB"] = np.std(child_img[:, :, 0:3])
                    self.images["child"][one_child]["std_pixel_A"] = np.std(child_img[:, :, 3])
    ...
    def show(self, 
        ...
        discrete_colorbars = [discrete_colorbar] if not (isinstance(discrete_colorbar, list)) else discrete_colorbar[:]
        ...
        if tree_level == "parent":
            for one_image_id in image_ids:
                ...
                plt.imshow(par_image, zorder=1, interpolation="nearest",
                           extent=(0, par_image.size[0], par_image.size[1], 0))

                if save_kml_dir:
                    if not "coord" in self.images["parent"][one_image_id].keys():
                        print(f"[WARNING] 'coord' could not be found. This is needed when save_kml_dir is set...continue")
                        continue
                    ...
                    plt.savefig(f"{path2kml}", bbox_inches='tight', pad_inches=0, dpi=kml_dpi_image)

                    self._createKML(path2kml=path2kml, 
                                    value=one_image_id, 
            ...
            for i in parents.keys():
                ...
                for i_value, value in enumerate(values):
                    ...
                    for child_id in parents[i]["child"]:
                        ...
                        if not value:
                            ...
                            image2plot[one_image["min_y"]:one_image["max_y"], 
                                       one_image["min_x"]:one_image["max_x"]] = 1.
                    ...
                    if value:
                        ...
                        plt.imshow(image2plot, 
                                   ...
                                   cmap=colorbar, vmin=vmin, vmax=vmax, alpha=alpha,
                ...
                if value and plot_histogram:
                    ...
                    plt.hist(image2plot.flatten(), 
                             color="k",
                             bins=np.arange(histogram_range[0] - histogram_range[2]/2., 
    ...
    def _createKML(self, path2kml, value, coords, counter=-1):
        ...
        try:
            ...
            raise ImportError("[ERROR] simplekml needs to be installed to create KML outputs!")
    ...
    def _plotBorder(self, image_dict, plt, linewidth=0.5, zorder=20, color="r"):
        ...
        """        
        plt.plot([image_dict["min_x"], image_dict["min_x"]], 
                 [image_dict["min_y"], image_dict["max_y"]], 
                 lw=linewidth, zorder=zorder, color=color)
        plt.plot([image_dict["min_x"], image_dict["max_x"]], 
                 [image_dict["max_y"], image_dict["max_y"]],
                 lw=linewidth, zorder=zorder, color=color)
        plt.plot([image_dict["max_x"], image_dict["max_x"]], 
                 [image_dict["max_y"], image_dict["min_y"]],
                 lw=linewidth, zorder=zorder, color=color)
        plt.plot([image_dict["max_x"], image_dict["min_x"]], 
    ...
    def loadPatches(self, 
        ...
        if parent_paths:
            # Add parents
            self.loadParents(parent_paths=parent_paths,
                             update=False, 
                             add_geo=add_geo_par)
    ...
    def detectBorderFromPath(image_id, border_delimiter="-"):
        ...
        return int(split_path[1]), int(split_path[2]), int(split_path[3]), int(split_path[4])
    
    def loadParents(self, parent_paths=False, parent_ids=False, update=False, add_geo=False):
        ...
        if parent_paths:
            ...
            for ppath in parent_paths:
                ...
                if os.path.isfile(ppath):
                    self.images["parent"][parent_id]["image_path"] = os.path.abspath(ppath)
                else:
                    self.images["parent"][parent_id]["image_path"] = None 
                if add_geo and ("tiff" in ppath):
                    lon_min, lon_max, lat_min, lat_max, tiff_shape, size_in_m = \
                        utils.extractGeoInfo(ppath, calc_size_in_m='great-circle')
                    self.images["parent"][parent_id]["coord"] = (lon_min, lon_max, lat_min, lat_max)
    ...
    def load_csv_file(self, parent_path=None, child_path=None, 
                      clear_images=False, 
                      index_col_child=0, 
                      index_col_parent=0):
        ...
        if isinstance(child_path, str) and os.path.isfile(child_path):
            self.images["child"].update(pd.read_csv(child_path, index_col=index_col_child).to_dict(orient="index"))
        if isinstance(parent_path, str) and os.path.isfile(parent_path):
            self.images["parent"].update(pd.read_csv(parent_path, index_col=index_col_parent).to_dict(orient="index"))
            self.addChildren()
            for one_par in self.images["parent"].keys():
                k2change = "children"
                if k2change in self.images["parent"][one_par]:
                    self.images["parent"][one_par][k2change] = eval(self.images["parent"][one_par][k2change])
