...
class TileMerger:
    ...
    def _generate_tile_name(self, index: GridIndex):
        ...
        str
            Tile file name
        """
        return "{}z={}_x={}_y={}.{}".format(self.temp_folder, index.z, index.x, index.y, self.img_input_format)
    ...
    def merge(self, grid_bb: GridBoundingBox, file_name: Union[str, None] = None):
        ...
        merged_image = Image.new('RGBA', (len(grid_bb.x_range) * tile_size, len(grid_bb.y_range) * tile_size))
        ...
        out_path = "{}{}.{}".format(self.output_folder, file_name, self.img_output_format[0])
        merged_image.save(out_path, self.img_output_format[1])
        logger.info("Merge successful! The image has been stored at '{}'".format(out_path))
