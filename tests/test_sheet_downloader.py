...
def test_extract_published_dates(sheet_downloader):
    ...
    )  # metadata has "1894 to 1896" - method take end date only (thoughts?)
...
def test_minmax_latlon(sheet_downloader, capfd):
    ...
    assert (
        out
        == "[INFO] Min lat: 51.49344796, max lat: 54.75000003 \n[INFO] Min lon: -5.40999994, max lon: -0.16093917\n"
    )
...
def test_download_by_string(sheet_downloader, tmp_path):
    ...
    sd.download_map_sheets_by_string(
        "one_inch", "id", maps_path, metadata_fname
    )  # test w/ keys string
    sd.download_map_sheets_by_string(
        "095", path_save=maps_path, metadata_fname=metadata_fname
    )  # test w/ no keys
