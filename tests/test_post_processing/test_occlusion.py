...
@pytest.fixture
def model():
    return timm.create_model("hf_hub:Livingwithmachines/mr_resnest101e_finetuned_OS_6inch_2nd_ed", pretrained=True)
...
def test_init_error(model):
    with pytest.raises(ValueError, match="as a string (path to csv file) or pd.DataFrame"):
...
def test_init_dataframe_no_predictions(patch_df, model):
    ...
    with pytest.raises(ValueError, match="patch dataframe should contain predicted labels"):
        OcclusionAnalyzer(patch_df, model)
