from mapreader import SheetDownloader, loader, AnnotationsLoader, ClassifierContainer, load_patches, PatchDataset
import pytest
...
def test_pipeline(tmp_path, sample_dir):
    ...
    my_ts = SheetDownloader(metadata_path=f"{sample_dir}/test_json.json",
    ...
    my_files.patchify_all(patch_size=300, # in pixels
                square_cuts=True, path_save=f"{tmp_path}/patches_300_pixel")
    ...
    annotated_images.load(f"{sample_dir}/land_#rw#.csv", images_dir=f"{tmp_path}/patches_300_pixel")
    ...
    #Classify - Inference
    my_maps = load_patches(f"{tmp_path}/patches_300_pixel/*74487492*png", 
